...
public class GraphicsDisplay extends JPanel {
    ...
    public boolean checkEven(Double []point)
    {
        double value = point[1]; // Получение значения функции в точке из массива

        int integerPart = (int) value; // Извлечение целой части значения функции

        double squareRoot = Math.sqrt(integerPart); // Вычисление квадратного корня целой части

        boolean isSquare = squareRoot == (int) squareRoot; // Проверка, является ли квадратный корень целым числом

        System.out.println("Целая часть значения функции в точке является квадратом целого числа: " + isSquare);
        return isSquare;
    }
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            canvas.draw(marker);
            Point2D.Double corner2 = shiftPoint2(center, 5.5, 5.5);
